...
def MUSIC(K, Y, n, SP):
    ...
    for i in range(180):
        a = np.exp(-1j * 2 * np.pi * d.reshape(-1, 1) * np.cos(Angles[i])/wavelength)  # 相位
...
def get_range_feature(adc_data, params):
    ...
    for i, frame in enumerate(adc_data):
        ...
        # (4) Angle Processing
        aoa_input_data_shift = np.squeeze(aoa_input_data[i, 0:params.numRxAntennas * 2, :, :])
        for j in range(params.numADCSamples):
            aoa_input_data_ = np.squeeze(frame_allVirtualAntenna[:, :, j])
            aoa_input_data_2 = aoa_input_data_.T
            SP = np.empty(180, dtype=complex)  # 按照给定维度生成数组 值不变
            azimuth_fft = MUSIC(K=8, Y=aoa_input_data_2, n=255, SP=SP)
        plt.title("Angle:90")
        plt.show()
        # aoa_input_data_1 = np.sum(np.abs(frame_allVirtualAntenna), axis=0)
        # num_max = np.argmax(aoa_input_data_1, axis=1)
        # aoa_input_data_2 = frame_allVirtualAntenna[:, :, num_max[0]]
        # aoa_input_data_2 = aoa_input_data_2.T
        # SP = np.empty(180, dtype=complex)  # 按照给定维度生成数组 值不变
        # azimuth_fft = MUSIC(K=8, Y=aoa_input_data_2, n=255, SP=SP)
